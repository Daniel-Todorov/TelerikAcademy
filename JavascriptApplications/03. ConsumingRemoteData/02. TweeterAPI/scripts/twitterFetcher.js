//NOTE!!! Since summer 2013 twitter changed their API and it's not really public anymore.
//You can't simply consume the data. You need to register developer account and then,
//using server side code, fetch the data using authenticating procedures.
//What I find is some piece of code that uses a workaround.
//The trick is to create a widget from your twitter account that fetches certain data and then use that widget to process the data.
//I guess it is possible to use node.js for the server logic too, but I am really not enough familiar with that technology.
		...
		twitterFetcher.fetch('492230362274078720', 'container', 10, true);